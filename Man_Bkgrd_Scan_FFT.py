...
centre_f = [9090e3, 9947.5e3, 11125e3, 11575e3, 13475e3, 13885e3, 16295e3, 18041e3, 19547.5e3, 19895e3]
low_freq = [9040e3, 9900e3, 11075e3, 11550e3, 13440e3, 13870e3, 16210e3, 18030e3, 19415e3, 19800e3]
high_freq = [9140e3, 9995e3, 11175e3, 11600e3, 13510e3, 13900e3, 16380e3, 18052e3, 19680e3, 19990e3]
...
for x in range (0,10):
    for y in range (0,20):
        ...
        # Trim file and convert to volts
        IQ = IQ [500:]; # Trim off first 500 samples (settling time)
        ...
        fftIQ = np.fft.fft(IQ)
        P = abs(fftIQ/L)
        P = np.fft.fftshift(P) # Shifts zero to centre
